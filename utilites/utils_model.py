...
def depth_selection_pnasnet(model, conv_kernel_size=3, n_layers = 3):
    conv_method = None
...
def depth_selection_pyramid_vgg(model,regularizer, conv_method=3, n_layers=1):
    # vgg conv5_1, conv5_3 , size is 37x37
    if conv_method == 3:
        conv = add_3by3_conv
    else:
    ...
    with tf.variable_scope('stack_v'):
        ...
        v_all = conv(v_all, n_layers, n_filters=[1024, 1024, 1024], name='v_all', regularizer=regularizer)
        # flattening image
...
def depth_selection(model, regularizer, conv_method=3, n_layers=1):
    # vgg conv5_1, conv5_3
    if conv_method == 3:
        conv = add_3by3_conv
    else:
    ...
    with tf.variable_scope('stack_v'):
        ...
        v4 = tf.identity(model['vgg_16/conv4/conv4_3'], name='v4')
        v4 = tf.image.resize_images(v4, size, method=resize_method)
        v4 = conv(v4, n_layers, n_filters=[1024, 1024, 1024], name='v4', regularizer=regularizer)
        v_all = tf.stack([v1, v2, v3, v4], axis=3)
        v_all = tf.reshape(v_all, [-1, v_all.shape[1] * v_all.shape[2], v_all.shape[3], v_all.shape[4]])
        v_all = tf.nn.l2_normalize(v_all, axis=-1, name='stacked_image_feature_maps')
    return v_all

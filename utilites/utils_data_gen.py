...
# Batch Gen Seg
def batch_gen_seg(valid_ids, annot_dict, train_images, n_batch):
    img_batch = np.empty((n_batch, 299, 299, 3), dtype='float32')
    cap_batch = []
    cat_batch = []
    msk_batch = np.empty((n_batch, 100, 37, 37))  # maximum 100 category per-image
    ...
    for i, chosen_id in enumerate(chosen_ids):
        ...
        for j, ann in enumerate(annot_dict[chosen_id]['annotations']):
            ...
            cat_sen.append(cat)
            msk_batch[i, j, :, :] = cv2.resize(annToMask(ann, size), (18, 18))
            # box_batch[i,j,:] = ann['bbox_norm']
        cat_batch.append(' '.join(cat_sen))
    return img_batch, cap_batch, cat_batch, msk_batch


def batch_gen_seg_add_cat_map(valid_ids, annot_dict, train_images, n_batch):
    #img_batch = np.empty((n_batch, 299, 299, 3), dtype='float32')
    #cap_batch = []
    cat_batch = []
    msk_batch = np.empty((n_batch, 100, 37, 37))  # maximum 100 category per-image
    ...
    for i, chosen_id in enumerate(chosen_ids):
        ...
        for k, v in cat_dict.items():
            cat_sen.append(k)
            np.clip(v, 0, 1, out=v)
            msk_batch[i, counter, :, :] = cv2.resize(v, (37, 37))
            counter += 1
        cat_batch.append(' '.join(cat_sen))
